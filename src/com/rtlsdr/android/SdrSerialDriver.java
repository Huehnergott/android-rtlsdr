...
import com.hoho.android.usbserial.driver.IUsbSerialPort;
...
/**
 * Was UsbSerialDriverOrig in bkhall/rtlsdr-android.
 * Renamed.
 * 
 * @author Huehnergott
 *
 */
public class SdrSerialDriver extends CommonUsbSerialDriver {
	...
	public static Map<Integer, int[]> getSupportedDevices() {
		...
		supportedDevices.put(Integer.valueOf(SdrUsbId.VENDOR_KYE), new int[] { SdrUsbId.KYE_RTL2832_707F });
		supportedDevices.put(Integer.valueOf(UsbId.VENDOR_RTL), new int[] { UsbId.RTL_RTL2838, UsbId.RTL_RTL2832 });
		supportedDevices.put(Integer.valueOf(UsbId.VENDOR_TERRATEC), new int[] { UsbId.TERRATEC_RTL2838_A9,
			UsbId.TERRATEC_RTL2838_B3, UsbId.TERRATEC_RTL2838_B4, UsbId.TERRATEC_RTL2838_B5,
			UsbId.TERRATEC_RTL2838_B7, UsbId.TERRATEC_RTL2838_B8, UsbId.TERRATEC_RTL2838_B9,
			UsbId.TERRATEC_RTL2838_C0, UsbId.TERRATEC_RTL2838_C6, UsbId.TERRATEC_RTL2838_D3,
			UsbId.TERRATEC_RTL2838_D7, UsbId.TERRATEC_RTL2838_E0 });
		supportedDevices.put(Integer.valueOf(SdrUsbId.VENDOR_COMPRO), new int[] { SdrUsbId.COMPRO_620,
				SdrUsbId.COMPRO_650, SdrUsbId.COMPRO_680 });

		supportedDevices.put(Integer.valueOf(SdrUsbId.VENDOR_AFATECH), new int[] { SdrUsbId.AFATECH_RTL2838_D393,
				SdrUsbId.AFATECH_RTL2838_D394, SdrUsbId.AFATECH_RTL2838_D395, SdrUsbId.AFATECH_RTL2838_D395,
				SdrUsbId.AFATECH_RTL2838_D396, SdrUsbId.AFATECH_RTL2838_D397, SdrUsbId.AFATECH_RTL2838_D398,
				SdrUsbId.AFATECH_RTL2838_D39D, SdrUsbId.AFATECH_RTL2838_D3A4 });

		supportedDevices.put(Integer.valueOf(SdrUsbId.VENDOR_DEXATEC), new int[] { SdrUsbId.DEXATEC_1101,
				SdrUsbId.DEXATEC_1102, SdrUsbId.DEXATEC_1103 });

		supportedDevices.put(Integer.valueOf(SdrUsbId.VENDOR_GTEK), new int[] { SdrUsbId.GTEK_RTL2838_B803,
				SdrUsbId.GTEK_RTL2838_C803, SdrUsbId.GTEK_RTL2838_D286, SdrUsbId.GTEK_RTL2838_D803 });
		return supportedDevices;
	}

	private static int libusb_control_transfer(byte requestType, byte request, char value, char index, byte[] buffer,
			char length, int timeout) {
		return mConnection.controlTransfer(requestType, request, value, index, buffer, length, timeout);
	}

	private static void Log_e(String tag, String message) {
		logInterface.log(tag, message);
	}
	...
	private static int rtlsdr_i2c_read(byte i2c_addr, byte[] buffer, byte len) {
		...
		return rtlsdr_read_array(IICB, (char) addr, buffer, len);
	}
	...
	private static int rtlsdr_read_array(byte iicb2, char addr, byte[] data, byte len) {
		int r;
		char index = (char) (iicb2 << 8);

		r = libusb_control_transfer(CTRL_IN, (byte) 0, addr, index, data, (char) len, USB_WRITE_TIMEOUT_MILLIS);

		if (r < 0) {
			Log_e("rtlsdr_read_array", " error" + len);
			Log_e("rtlsdr_read_array", "addr 0x" + Integer.toHexString(addr) + " index 0x" + Integer.toHexString(index)
					+ " array " + data[0] + " len " + len);
		}
		return r;
	}

	private static int rtlsdr_write_array(byte iicb2, char addr, byte[] array, byte len) {
		int r;
		char index = (char) ((iicb2 << 8) | 0x10);

		r = libusb_control_transfer(CTRL_OUT, (byte) 0, addr, index, array, (char) len, USB_WRITE_TIMEOUT_MILLIS);

		if (r < 0) {
			Log_e("rtlsdr_write_array",
					"addr 0x" + Integer.toHexString(addr) + " index 0x" + Integer.toHexString(index) + " array "
							+ array[0] + " len " + len);
			Log_e("rtlsdr_write_array", " error byte count " + len);
		}
		return r;
	}
	...
	private static ILogger logInterface = new DefaultLogger();

	private static final String TAG = SdrSerialDriver.class.getSimpleName();
	...
	private int gain = this.MODES_MAX_GAIN;
	private boolean enable_agc = false;

	private long freq = this.MODES_DEFAULT_FREQ;
	// Buffersizes
	...
	// private static final byte i2c_addr = 0; //F.I.X.M.E
	...
	private long rate = this.MODES_DEFAULT_RATE;
	...
	// AdsbParse adsb;
	public SdrSerialDriver(UsbDevice device, UsbDeviceConnection connection) {
		...
		try {
			for (int i = 0; i < this.mDevice.getInterfaceCount(); i++) {
				...
				if (mConnection.claimInterface(usbIface, true)) {
					Log_e(TAG, "claimInterface " + i + " SUCCESS");
				} else {
					Log_e(TAG, "claimInterface " + i + " FAIL");
				}
			}
			int count = this.mDevice.getInterfaceCount();
			for (int i = 0; i < count; i++) {
				UsbInterface intf = this.mDevice.getInterface(i);
				if (intf != null) {
					Log_e(TAG, "intf.getInterfaceClass()=" + i + " " + intf.getInterfaceClass()
							+ " intf.getInterfaceSubclass()=" + i + " " + intf.getInterfaceSubclass()
							+ " intf.getInterfaceProtocol()=" + i + " " + intf.getInterfaceProtocol());
					Log_e(TAG, "dataIface.getEndpointCount " + i + " " + intf.getEndpointCount());
				}
				int getEndpointCount = intf.getEndpointCount();
				for (int j = 0; j < getEndpointCount; j++) {
					UsbEndpoint ep = intf.getEndpoint(j);
					Log_e(TAG, "found endpoint getEndpointNumber= " + ep.getEndpointNumber());
					Log_e(TAG, "found endpoint type= " + ep.getType());
					Log_e(TAG, "found endpoint getDirection= " + ep.getDirection());
					Log_e(TAG, "found endpoint getMaxPacketSize= " + ep.getMaxPacketSize());

					if (ep.getType() == UsbConstants.USB_ENDPOINT_XFER_BULK) {
						if (ep.getDirection() == UsbConstants.USB_DIR_IN) {
							this.mReadEndpoint = ep;
							Log_e(TAG, "found READ USB_ENDPOINT_XFER_BULK");
						} else {
							Log_e(TAG, "found WRITE USB_DIR_OUT->USB_ENDPOINT_XFER_BULK");
						}
					}
				}
				opened = true;
	...
	public int read(byte[] dest, int timeoutMillis) throws IOException {
		...
		synchronized (this.mReadBufferLock) {
			...
			totalBytesRead = mConnection.bulkTransfer(this.mReadEndpoint, this.mReadBuffer, readAmt,
					timeoutMillis * 1000);
			...
			// }
			if (totalBytesRead < 0) {
				Log_e(TAG, " mConnection.bulkTransfer ERROR totalBytesRead=" + totalBytesRead);

			} else {
				System.arraycopy(this.mReadBuffer, 0, dest, 0, totalBytesRead);
			}
	...
	private int rtlsdr_deinit_baseband() {
		...
		if (ret != 0) {
			Log_e(TAG, " ret " + ret + "Line " + Thread.currentThread().getStackTrace()[2].getLineNumber());
		}
	...
	private char rtlsdr_demod_read_reg(byte page, char addr, byte len) {
		...
		if (r < 0) {
			Log_e(TAG, "rtlsdr_demod_read_reg error " + r);
			Log_e(TAG,
					" addr " + Integer.toHexString(addr) + " index " + Integer.toHexString(index) + " data[0] "
							+ Integer.toHexString(data[0]) + " data[1] " + Integer.toHexString(data[1]) + " len "
	...
	private int rtlsdr_demod_write_reg(byte page, char addr, char val, byte len) {
		...
		r = libusb_control_transfer(CTRL_OUT, (byte) 0, addr, index, data, (char) len, USB_WRITE_TIMEOUT_MILLIS);

		if (r < 0) {
			Log_e(TAG, "rtlsdr_demod_write_reg failed " + r);
			Log_e(TAG,
					" addr " + Integer.toHexString(addr) + " index " + Integer.toHexString(index) + " data[0] "
							+ Integer.toHexString(data[0]) + " data[1] " + Integer.toHexString(data[1]) + " len "
							+ Integer.toHexString(len));
	...
	private IRtlSdrTuner rtlsdr_detect() throws IOException {
		...
		if (myTuner != null) {
			Log_e(TAG, "Error Already called rtlsdr_open()");
			// return (myTuner);
		}

		Log_e(TAG, "rtlsdr_open()");

		/* perform a dummy write, if it fails, reset the device */
		if ((ret = rtlsdr_write_reg(USBB, USB_SYSCTL, (char) 0x09, (byte) 1)) < 0) {
			if (ret != 0) {
				Log_e(TAG, "ERROR ret " + ret + " Line " + Thread.currentThread().getStackTrace()[2].getLineNumber());
			}
			Log_e(TAG, "Resetting device...");
		...
		if (reg == R820T_CHECK_VAL) {
			Log_e(TAG, "Found Rafael Micro R820T tuner\n");
		...
		if (reg == R820T_CHECK_VAL) {
			Log_e(TAG, "Found Rafael Micro R828D tuner\n");
		...
		if (reg == E4K_CHECK_VAL) {
			Log_e(TAG, "Found Elonics E4000 tuner");
		...
		if (reg == FC0013_CHECK_VAL) {
			Log_e(TAG, "Found Fitipower FC0013 tuner");
		...
		if (this.tuner_type == RTLSDR_TUNER_UNKNOWN) {
			Log_e(TAG, "No supported tuner found\n");
	...
	private int rtlsdr_get_tuner_gains(int[] dst) {
		...
		int[] e4k_gains = { -10, 15, 40, 65, 90, 115, 140, 165, 190, 215, 240, 290, 340, 420 };
		int[] fc0012_gains = { -99, -40, 71, 179, 192 };
		int[] fc0013_gains = { -99, -73, -65, -63, -60, -58, -54, 58, 61, 63, 65, 67, 68, 70, 71, 179, 181, 182, 184,
				186, 188, 191, 197 };
		int[] fc2580_gains = { 0 /* no gain values */};
		int[] r820t_gains = { 0, 9, 14, 27, 37, 77, 87, 125, 144, 157, 166, 197, 207, 229, 254, 280, 297, 328, 338,
	...
	private void rtlsdr_init_baseband() {
		 ...
		 */
		char[] fir_coeff = { 0xca, 0xdc, 0xd7, 0xd8, 0xe0, 0xf2, 0x0e, 0x35, 0x06, 0x50, 0x9c, 0x0d, 0x71, 0x11, 0x14,
	...
	private int rtlsdr_read_eeprom(byte[] data, byte[] offset, char len) {
		...
		if ((len + offset[0]) > 256) {
			Log_e(TAG, " rtlsdr_read_eeprom offset + len > 256");
			return -2;
		}
		...
		for (i = 0; i < len; i++) {
			...
			if (r < 0) {
				Log_e(TAG, " rtlsdr_read_eeprom FAILED");
				return -3;
			}
	...
	private char rtlsdr_read_reg(byte block, char addr, byte len) {
		...
		if (r < 0) {
			Log_e(TAG, "rtlsdr_read_reg failed " + r);
		}
	...
	private int rtlsdr_read_sync(byte[] buf, int len, int n_read) throws IOException {
	...
	private int rtlsdr_set_agc_mode(boolean on) {
		...
		return rtlsdr_demod_write_reg((byte) 0, (char) 0x19, (char) (on ? 0x25 : 0x05), (byte) 1);
	}
	...
	private int rtlsdr_set_direct_sampling(boolean on) {
		...
		if (on) {
			if ((this.tuner) && this.tuner_exit) {
				...
				if (myTuner != null) {
					try {
						...
						Log_e(TAG, "myTuner.exit(0) FAILED " + e.toString());
						e.printStackTrace();
					}
				}
				rtlsdr_set_i2c_repeater(false);
			...
			/* disable Zero-IF mode */
			r |= rtlsdr_demod_write_reg((byte) 1, (char) 0xb1, (char) 0x1a, (byte) 1);

			/* disable spectrum inversion */
			r |= rtlsdr_demod_write_reg((byte) 1, (char) 0x15, (char) 0x00, (byte) 1);

			/* only enable In-phase ADC input */
			r |= rtlsdr_demod_write_reg((byte) 0, (char) 0x08, (char) 0x4d, (byte) 1);

			/* swap I and Q ADC, this allows to select between two inputs */
			r |= rtlsdr_demod_write_reg((byte) 0, (char) 0x06, (char) (on ? 0x90 : 0x80), (byte) 1);

			Log_e(TAG, "rtlsdr_set_direct_sampling Enabled direct sampling mode, input " + on);
			this.direct_sampling = on;
		} else {
			if ((myTuner != null) && this.tuner_init) {
				rtlsdr_set_i2c_repeater(true);
				try {
					r |= myTuner.init(0);
				} catch (IOException e) {
			...
			if (this.tuner_type == RTLSDR_TUNER_R820T) {
				...
				/* enable spectrum inversion */
				r |= rtlsdr_demod_write_reg((byte) 1, (char) 0x15, (char) 0x01, (byte) 1);
			} else {
				r |= rtlsdr_set_if_freq(0);

				/* enable In-phase + Quadrature ADC input */
				r |= rtlsdr_demod_write_reg((byte) 0, (char) 0x08, (char) 0xcd, (byte) 1);

				/* Enable Zero-IF mode */
				r |= rtlsdr_demod_write_reg((byte) 1, (char) 0xb1, (char) 0x1b, (byte) 1);
			}

			/* opt_adc_iq = 0, default ADC_I/ADC_Q datapath */
			r |= rtlsdr_demod_write_reg((byte) 0, (char) 0x06, (char) 0x80, (byte) 1);

			Log_e(TAG, "Disabled direct sampling mode\n");
	...
	private void rtlsdr_set_i2c_repeater(boolean on) {
		rtlsdr_demod_write_reg((byte) 1, (char) 0x01, (char) (on ? 0x18 : 0x10), (byte) 1);
	}
	...
	private int rtlsdr_set_sample_rate(long samp_rate) {
		...
		if (samp_rate != real_rate) {
			Log_e(TAG, "Exact sample rate is: " + real_rate + " Hz");
		}
		...
		if ((myTuner != null) && this.tuner_set_bw) {
			...
			if (myTuner != null) {
				try {
					...
					Log_e(TAG, "error set_bw " + real_rate);
					e.printStackTrace();
				}
			}
			rtlsdr_set_i2c_repeater(false);
		...
		/* reset demod (bit 3, soft_rst) */
		r |= rtlsdr_demod_write_reg((byte) 1, (char) 0x01, (char) 0x14, (byte) 1);
		r |= rtlsdr_demod_write_reg((byte) 1, (char) 0x01, (char) 0x10, (byte) 1);
	...
	private int rtlsdr_set_testmode(boolean on) {
		...
		return rtlsdr_demod_write_reg((byte) 0, (char) 0x19, (char) (on ? 0x03 : 0x05), (byte) 1);
	}
	...
	private int rtlsdr_set_tuner_gain(int inputGain) {
		...
		if (this.tuner_set_gain) {
			...
			try {
				...
				Log_e(TAG, "set_gain error " + e.toString());
				e.printStackTrace();
			}
			rtlsdr_set_i2c_repeater(false);
		}
	...
	private int rtlsdr_write_eeprom(byte[] data, char offset, char len) throws InterruptedException {
		int r = 0;
		int i;
		byte[] cmd = new byte[2];
	...
	private int rtlsdr_write_reg(byte block, char addr, char val, byte len) {
		...
		if (r < 0) {
			Log_e("rtlsdr_write_reg", "block " + block + " addr " + " val " + " len " + len);
			Log_e("rtlsdr_write_reg", "Line " + Thread.currentThread().getStackTrace()[2].getLineNumber());
		}
		return r;
	}
	...
	private int setConfigSingle(int request, int value) {
		return mConnection
				.controlTransfer(REQTYPE_HOST_TO_DEVICE, request, value, 0, null, 0, USB_WRITE_TIMEOUT_MILLIS);
	}
	...
	private void StartDevice() {
		...
		rtlsdr_set_tuner_gain_mode((this.gain == this.MODES_AUTO_GAIN) ? true : false);
		if (this.gain != this.MODES_AUTO_GAIN) {
			if (this.gain == this.MODES_MAX_GAIN) {
				/* Find the maximum gain available. */
				int numgains;
				int[] gains = new int[100];

				numgains = rtlsdr_get_tuner_gains(gains);
				this.gain = gains[numgains - 1];
				Log_e(TAG, "Max available gain is: " + String.format("%.2f", this.gain / 10.0));
			}
			rtlsdr_set_tuner_gain(this.gain);
			Log_e(TAG, "Setting gain to: " + String.format("%.2f", this.gain / 10.0));
		} else {
			Log_e(TAG, "Using automatic gain control.\n");
		}
		rtlsdr_set_freq_correction(this.ppm_error);
		if (this.enable_agc) {
			rtlsdr_set_agc_mode(true);
			Log_e(TAG, "AGC mode is on");
		} else {
			Log_e(TAG, "AGC mode is off");
		}

		rtlsdr_set_center_freq(this.freq);
		// rtlsdr_set_tuner_gain_mode(true);//true == AGC
		rtlsdr_set_sample_rate(this.MODES_DEFAULT_RATE);
		rtlsdr_reset_buffer();
		Log_e(TAG, "Gain reported by device: " + String.format("%.2f", rtlsdr_get_tuner_gain() / 10.0));
		return;
	}
	...
	public int write(byte[] src, int timeoutMillis) throws IOException {
		...
		Log_e(TAG, "Writes not support yet writeLength= " + src.length);
