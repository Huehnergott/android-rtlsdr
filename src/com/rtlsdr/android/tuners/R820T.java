...
public class R820T implements IRtlSdrTuner {
	...
	byte[] R828_iniArry = { (byte) 0x83, 0x32, 0x75, (byte) 0xC0, 0x40, (byte) 0xD6, 0x6C, (byte) 0xF5, 0x63,
			/* 0x05 0x06 0x07 0x08 0x09 0x0A 0x0B 0x0C 0x0D */

			0x75, 0x68, 0x6C, (byte) 0x83, (byte) 0x80, 0x00, 0x0F, 0x00, (byte) 0xC0,// xtal_check
			/* 0x0E 0x0F 0x10 0x11 0x12 0x13 0x14 0x15 0x16 */

			0x30, 0x48, (byte) 0xCC, 0x60, 0x00, 0x54, (byte) 0xAE, 0x4A, (byte) 0xC0 };;
	...
	static int[] r820t_vga_gain_steps = { 0, 26, 26, 30, 42, 35, 24, 13, 14, 32, 36, 34, 35, 37, 35, 36 };

	static int[] r820t_lna_gain_steps = { 0, 9, 13, 40, 38, 13, 31, 22, 26, 31, 26, 14, 19, 5, 35, 13 };

	static int[] r820t_mixer_gain_steps = { 0, 5, 10, 10, 19, 9, 10, 25, 17, 10, 8, 16, 13, 6, 3, -8 };
	...
	boolean I2C_Read_Len(int pTuner, R828_I2C_LEN_TYPE I2C_Info) {
		...
		if (SdrSerialDriver.rtlsdr_i2c_write_fn(R820T_I2C_ADDR, RegStartAddr, (byte) 1) < 0) {
			return false;
		}
		...
		if (SdrSerialDriver.rtlsdr_i2c_read_fn(R820T_I2C_ADDR, ReadingBytes, ByteNum) < 0) {
			return false;
		}
	...
	boolean I2C_Write(int pTuner, R828_I2C_TYPE I2C_Info) {
		...
		if (SdrSerialDriver.rtlsdr_i2c_write_fn(R820T_I2C_ADDR, WritingBuffer, (byte) 2) < 0) {
			return false;
		}
	...
	boolean I2C_Write_Len(int pTuner, R828_I2C_LEN_TYPE I2C_Info) {
		...
		for (i = 0; i < ByteNum; i += WritingByteNumMax) {
			...
			// FUNCTION_SUCCESS)
			// goto error_status_set_tuner_registers;
			Log.d(TAG, "WritingBuffer[0] " + WritingBuffer[0] + "," + " WritingBuffer[1] " + WritingBuffer[1] + " i= "
							+ i);
			while (SdrSerialDriver.rtlsdr_i2c_write_fn(R820T_I2C_ADDR, WritingBuffer, (byte) (WritingByteNum + 1)) < 0) {
				Log.d(TAG, "WritingBuffer[0] " + WritingBuffer[0] + "," + " WritingBuffer[1] " + WritingBuffer[1]
								+ " i= " + i + " FAILED!!!");
				int count = 0;
				count++;
				if (count > 5) {
	...
	boolean R828_CompreStep(int pTuner, R828_SectType[] StepArry, byte Pace) {
		...
		while (((StepTemp.Gain_X & 0x1F) < IMR_TRIAL) && ((StepTemp.Phase_Y & 0x1F) < IMR_TRIAL)) // 5->10
		{
			...
			this.R828_I2C.RegAddr = 0x08;
			this.R828_I2C.Data = (byte) StepTemp.Gain_X;
			if (I2C_Write(pTuner, this.R828_I2C) != true) {
				return false;
			}

			this.R828_I2C.RegAddr = 0x09;
			this.R828_I2C.Data = (byte) StepTemp.Phase_Y;
			if (I2C_Write(pTuner, this.R828_I2C) != true) {
				return false;
			}
	...
	boolean R828_F_IMR(int pTuner, R828_SectType[] IQ_Pont) {
		...
		for (VGA_Count = 12; VGA_Count < 16; VGA_Count++) {
			this.R828_I2C.RegAddr = 0x0C;
			this.R828_I2C.Data = (byte) ((this.R828_Arry[7] & 0xF0) + VGA_Count);
			if (I2C_Write(pTuner, this.R828_I2C) != true) {
				return false;
			}
		...
		if (R828_IQ_Tree(pTuner, Compare_IQ[0].Gain_X, Compare_IQ[0].Phase_Y, 0x08, Compare_IQ) != true) {
			return false;
		}
	...
	boolean R828_Filt_Cal(int pTuner, int Cal_Freq, int R828_BW) {
		...
		// Set filt_cap
		this.R828_I2C.RegAddr = 0x0B;
		this.R828_Arry[6] = (byte) ((this.R828_Arry[6] & 0x9F) | (Sys_Info1.HP_COR & 0x60));
		...
		// set cali clk =on
		this.R828_I2C.RegAddr = 0x0F; // reg15
		this.R828_Arry[10] |= 0x04; // calibration clk=on
		...
		this.R828_I2C.RegAddr = 0x10;
		this.R828_Arry[11] = (byte) ((this.R828_Arry[11] & 0xFC) | 0x00);
		...
		// Start Trigger
		this.R828_I2C.RegAddr = 0x0B; // reg11
		this.R828_Arry[6] |= 0x10; // vstart=1
		this.R828_I2C.Data = this.R828_Arry[6];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
		...
		// Stop Trigger
		this.R828_I2C.RegAddr = 0x0B;
		this.R828_Arry[6] &= 0xEF; // vstart=0
		this.R828_I2C.Data = this.R828_Arry[6];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
		...
		// set cali clk =off
		this.R828_I2C.RegAddr = 0x0F; // reg15
		this.R828_Arry[10] &= 0xFB; // calibration clk=off
		this.R828_I2C.Data = this.R828_Arry[10];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
	...
	boolean R828_GetRfGain(int pTuner, R828_RF_Gain_Info[] pR828_rf_gain) {
		...
		this.R828_I2C_Len.RegAddr = 0x00;
		this.R828_I2C_Len.Len = 4;
		if (I2C_Read_Len(pTuner, this.R828_I2C_Len) != true) {
			return false;
		}

		pR828_rf_gain[0].RF_gain1 = (byte) (this.R828_I2C_Len.Data[3] & 0x0F);
		pR828_rf_gain[0].RF_gain2 = (byte) ((this.R828_I2C_Len.Data[3] & 0xF0) >> 4);
	...
	boolean R828_GPIO(int pTuner, /* R828_GPIO_Type */boolean R828_GPIO_Conrl) {
		if (R828_GPIO_Conrl == /* HI_SIG */true) {
			this.R828_Arry[10] |= 0x01;
		} else {
			this.R828_Arry[10] &= 0xFE;
		}

		this.R828_I2C.RegAddr = 0x0F;
		this.R828_I2C.Data = this.R828_Arry[10];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
	...
	boolean R828_IMR(int pTuner, int IMR_MEM, boolean IM_Flag) {
		...
		this.R828_Arry[19] &= 0xF0; // set ring[3:0]
		this.R828_Arry[19] |= n_ring;
		RingVCO = (16 + n_ring) * 8 * RingRef;
		this.R828_Arry[19] &= 0xDF; // clear ring_se23
		this.R828_Arry[20] &= 0xFC; // clear ring_seldiv
		this.R828_Arry[26] &= 0xFC; // clear ring_att

		switch (IMR_MEM) {
		case 0:
			RingFreq = RingVCO / 48;
			this.R828_Arry[19] |= 0x20; // ring_se23 = 1
			this.R828_Arry[20] |= 0x03; // ring_seldiv = 3
			this.R828_Arry[26] |= 0x02; // ring_att 10
			break;
		case 1:
			RingFreq = RingVCO / 16;
			this.R828_Arry[19] |= 0x00; // ring_se23 = 0
			this.R828_Arry[20] |= 0x02; // ring_seldiv = 2
			this.R828_Arry[26] |= 0x00; // pw_ring 00
			break;
		case 2:
			RingFreq = RingVCO / 8;
			this.R828_Arry[19] |= 0x00; // ring_se23 = 0
			this.R828_Arry[20] |= 0x01; // ring_seldiv = 1
			this.R828_Arry[26] |= 0x03; // pw_ring 11
			break;
		case 3:
			RingFreq = RingVCO / 6;
			this.R828_Arry[19] |= 0x20; // ring_se23 = 1
			this.R828_Arry[20] |= 0x00; // ring_seldiv = 0
			this.R828_Arry[26] |= 0x03; // pw_ring 11
			break;
		case 4:
			RingFreq = RingVCO / 4;
			this.R828_Arry[19] |= 0x00; // ring_se23 = 0
			this.R828_Arry[20] |= 0x00; // ring_seldiv = 0
			this.R828_Arry[26] |= 0x01; // pw_ring 01
			break;
		default:
			RingFreq = RingVCO / 4;
			this.R828_Arry[19] |= 0x00; // ring_se23 = 0
		...
		// ------------n_ring,ring_se23----------//
		this.R828_I2C.RegAddr = 0x18;
		this.R828_I2C.Data = this.R828_Arry[19];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
		// ------------ring_sediv----------------//
		this.R828_I2C.RegAddr = 0x19;
		this.R828_I2C.Data = this.R828_Arry[20];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
		// ------------pw_ring-------------------//
		this.R828_I2C.RegAddr = 0x1f;
		...
		// Must do before PLL()
		if (R828_MUX(pTuner, RingFreq - 5300) != true) {
			// Freq
			return false;
		}

		if (R828_PLL(pTuner, (RingFreq - 5300) * 1000, STD_SIZE) != true) {
			// pll
					// freq
					// =
					// ring
		...
		if (IM_Flag == true) {
			...
			IMR_POINT[0].Gain_X = this.IMR_Data[3].Gain_X;
			IMR_POINT[0].Phase_Y = this.IMR_Data[3].Phase_Y;
			IMR_POINT[0].Value = this.IMR_Data[3].Value;
		...
		switch (IMR_MEM) {
		case 0:
			this.IMR_Data[0].Gain_X = IMR_POINT[0].Gain_X;
			this.IMR_Data[0].Phase_Y = IMR_POINT[0].Phase_Y;
			this.IMR_Data[0].Value = IMR_POINT[0].Value;
			break;
		case 1:
			this.IMR_Data[1].Gain_X = IMR_POINT[0].Gain_X;
			this.IMR_Data[1].Phase_Y = IMR_POINT[0].Phase_Y;
			this.IMR_Data[1].Value = IMR_POINT[0].Value;
			break;
		case 2:
			this.IMR_Data[2].Gain_X = IMR_POINT[0].Gain_X;
			this.IMR_Data[2].Phase_Y = IMR_POINT[0].Phase_Y;
			this.IMR_Data[2].Value = IMR_POINT[0].Value;
			break;
		case 3:
			this.IMR_Data[3].Gain_X = IMR_POINT[0].Gain_X;
			this.IMR_Data[3].Phase_Y = IMR_POINT[0].Phase_Y;
			this.IMR_Data[3].Value = IMR_POINT[0].Value;
			break;
		case 4:
			this.IMR_Data[4].Gain_X = IMR_POINT[0].Gain_X;
			this.IMR_Data[4].Phase_Y = IMR_POINT[0].Phase_Y;
			this.IMR_Data[4].Value = IMR_POINT[0].Value;
			break;
		default:
			this.IMR_Data[4].Gain_X = IMR_POINT[0].Gain_X;
			this.IMR_Data[4].Phase_Y = IMR_POINT[0].Phase_Y;
	...
	boolean R828_IMR_Cross(int pTuner, R828_SectType[] IQ_Pont, byte[] X_Direct) {
		...
		CrossCount = 0;
		Reg08 = (byte) (this.R828_iniArry[3] & 0xC0);
		Reg09 = (byte) (this.R828_iniArry[4] & 0xC0);
		...
		for (CrossCount = 0; CrossCount < 5; CrossCount++) {
			...
			this.R828_I2C.RegAddr = 0x08;
			this.R828_I2C.Data = (byte) Compare_Cross[CrossCount].Gain_X;
			if (I2C_Write(pTuner, this.R828_I2C) != true) {
				return false;
			}

			this.R828_I2C.RegAddr = 0x09;
			this.R828_I2C.Data = (byte) Compare_Cross[CrossCount].Phase_Y;
			if (I2C_Write(pTuner, this.R828_I2C) != true) {
				return false;
			}
...

	{
		...
		for (ArrayNum = 0; ArrayNum < 27; ArrayNum++) {
			this.R828_Arry[ArrayNum] = this.R828_iniArry[ArrayNum];
		}
		// IMR Preparation
		// lna off (air-in off)
		this.R828_I2C.RegAddr = 0x05;
		this.R828_Arry[0] = (byte) (this.R828_Arry[0] | 0x20);
		this.R828_I2C.Data = this.R828_Arry[0];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
		// mixer gain mode = manual
		this.R828_I2C.RegAddr = 0x07;
		this.R828_Arry[2] = (byte) (this.R828_Arry[2] & 0xEF);
		this.R828_I2C.Data = this.R828_Arry[2];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
		// filter corner = lowest
		this.R828_I2C.RegAddr = 0x0A;
		this.R828_Arry[5] = (byte) (this.R828_Arry[5] | 0x0F);
		...
		this.R828_I2C.RegAddr = 0x0B;
		this.R828_Arry[6] = (byte) ((this.R828_Arry[6] & 0x90) | 0x60);
		this.R828_I2C.Data = this.R828_Arry[6];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
		...
		this.R828_I2C.RegAddr = 0x0C;
		this.R828_Arry[7] = (byte) ((this.R828_Arry[7] & 0x60) | 0x0B);
		...
		// ring power = on
		this.R828_I2C.RegAddr = 0x18;
		this.R828_Arry[19] = (byte) (this.R828_Arry[19] | 0x10);
		this.R828_I2C.Data = this.R828_Arry[19];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
		// from ring = ring pll in
		this.R828_I2C.RegAddr = 0x1C;
		this.R828_Arry[23] = (byte) (this.R828_Arry[23] | 0x02);
		this.R828_I2C.Data = this.R828_Arry[23];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
		// sw_pdect = det3
		this.R828_I2C.RegAddr = 0x1E;
		this.R828_Arry[25] = (byte) (this.R828_Arry[25] | 0x80);
		this.R828_I2C.Data = this.R828_Arry[25];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
		// Set filt_3dB
		this.R828_Arry[1] = (byte) (this.R828_Arry[1] | 0x20);
		this.R828_I2C.RegAddr = 0x06;
	...
	boolean R828_Init(int pTuner) {
		...
		if (this.R828_IMR_done_flag == false) {
			...
			// Do Xtal check
			if ((this.Rafael_Chip == R820T) || (this.Rafael_Chip == R828S) || (this.Rafael_Chip == R820C)) {
				this.Xtal_cap_sel = XTAL_HIGH_CAP_0P;
			} else {
				if (R828_Xtal_Check(pTuner) != true) {
					return false;
				}
			...
			for (i = 0; i < STD_SIZE; i++) {
				this.R828_Fil_Cal_flag[i] = 0;// FALSE=0
	...
	boolean R828_InitReg(int pTuner) {
		...
		// Write Full Table
		this.R828_I2C_Len.RegAddr = 0x05;
		this.R828_I2C_Len.Len = (byte) InitArryNum;
		for (InitArryCount = 0; InitArryCount < InitArryNum; InitArryCount++) {
			this.R828_I2C_Len.Data[InitArryCount] = this.R828_iniArry[InitArryCount];
		}
		if (I2C_Write_Len(pTuner, this.R828_I2C_Len) != true) {
			return false;
		}
	...
	boolean R828_IQ(int pTuner, R828_SectType[] IQ_Pont) {
		...
		Compare_IQ[0].Gain_X = this.R828_iniArry[3] & 0xC0; // Jason modified,
		// clear
				// b[5], b[4:0]
		Compare_IQ[0].Phase_Y = this.R828_iniArry[4] & 0xC0; //
		...
		if (X_Direction[0] == 1) {
			...
			// increase step to find min value of this direction
			if (R828_CompreStep(pTuner, Compare_IQ, (byte) 0x08) != true) {
				return false;
			}
		} else {
			// compare and find min of 3 points. determine I/Q direction
			...
			// increase step to find min value of this direction
			if (R828_CompreStep(pTuner, Compare_IQ, (byte) 0x09) != true) {
				return false;
			}
		}
		/*
		...
		if (X_Direction[0] == 1) {
			...
			if (R828_IQ_Tree(pTuner, Compare_IQ[0].Phase_Y, Compare_IQ[0].Gain_X, 0x09, Compare_IQ) != true) {
				return false;
			}
		...
		this.R828_I2C.RegAddr = 0x08;
		this.R828_I2C.Data = (byte) (this.R828_iniArry[3] & 0xC0); // Jason
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}

		this.R828_I2C.RegAddr = 0x09;
		this.R828_I2C.Data = (byte) (this.R828_iniArry[4] & 0xC0);
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
	...
	// --------------------------------------------------------------------------------------------
	boolean R828_IQ_Tree(int pTuner, int FixPot, int FlucPot, int PotReg, R828_SectType[] CompareTree) {
		byte TreeCount;
		byte TreeTimes;
		byte TempPot;
		byte PntReg;
		...
		for (TreeCount = 0; TreeCount < TreeTimes; TreeCount++) {
			this.R828_I2C.RegAddr = (byte) PotReg;
			this.R828_I2C.Data = (byte) FixPot;
			if (I2C_Write(pTuner, this.R828_I2C) != true) {
				return false;
			}

			this.R828_I2C.RegAddr = PntReg;
			this.R828_I2C.Data = (byte) FlucPot;
			if (I2C_Write(pTuner, this.R828_I2C) != true) {
				return false;
			}
			char[] value = new char[1];
	...
	// modified
	{
		...
		for (ReadCount = 0; ReadCount < 6; ReadCount++) {
			this.R828_I2C_Len.RegAddr = 0x00;
			this.R828_I2C_Len.Len = (byte) (IMR_Reg + 1); // IMR_Reg = 0x01
			if (I2C_Read_Len(pTuner, this.R828_I2C_Len) != true) {
				return false;
			}

			ReadData = this.R828_I2C_Len.Data[1];
	...
	boolean R828_MUX(int pTuner, int RF_KHz) {
		...
		// Open Drain
		this.R828_I2C.RegAddr = 0x17;
		this.R828_Arry[18] = (byte) ((this.R828_Arry[18] & 0xF7) | Freq_Info1.OPEN_D);
		...
		// RF_MUX,Polymux
		this.R828_I2C.RegAddr = 0x1A;
		this.R828_Arry[21] = (byte) ((this.R828_Arry[21] & 0x3C) | Freq_Info1.RF_MUX_PLOY);
		...
		// TF BAND
		this.R828_I2C.RegAddr = 0x1B;
		this.R828_Arry[22] &= 0x00;
		this.R828_Arry[22] |= Freq_Info1.TF_C;
		...
		// XTAL CAP & Drive
		this.R828_I2C.RegAddr = 0x10;
		this.R828_Arry[11] &= 0xF4;
		switch (this.Xtal_cap_sel) {
		case XTAL_LOW_CAP_30P:
		case XTAL_LOW_CAP_20P:
			this.R828_Arry[11] = (byte) (this.R828_Arry[11] | Freq_Info1.XTAL_CAP20P | 0x08);
			break;

		case XTAL_LOW_CAP_10P:
			this.R828_Arry[11] = (byte) (this.R828_Arry[11] | Freq_Info1.XTAL_CAP10P | 0x08);
			break;

		case XTAL_LOW_CAP_0P:
			this.R828_Arry[11] = (byte) (this.R828_Arry[11] | Freq_Info1.XTAL_CAP0P | 0x08);
			break;

		case XTAL_HIGH_CAP_0P:
			this.R828_Arry[11] = (byte) (this.R828_Arry[11] | Freq_Info1.XTAL_CAP0P | 0x00);
			break;
		...
		// Set_IMR
		if (this.R828_IMR_done_flag == true) {
			RT_Reg08 = this.IMR_Data[Freq_Info1.IMR_MEM].Gain_X & 0x3F;
			RT_Reg09 = this.IMR_Data[Freq_Info1.IMR_MEM].Phase_Y & 0x3F;
		} else {
			RT_Reg08 = 0;
			RT_Reg09 = 0;
		}
		...
		this.R828_I2C.RegAddr = 0x08;
		this.R828_Arry[3] = (byte) (this.R828_iniArry[3] & 0xC0);
		this.R828_Arry[3] = (byte) (this.R828_Arry[3] | RT_Reg08);
		...
		this.R828_I2C.RegAddr = 0x09;
		this.R828_Arry[4] = (byte) (this.R828_iniArry[4] & 0xC0);
		this.R828_Arry[4] = (byte) (this.R828_Arry[4] | RT_Reg09);
	...
	boolean R828_PLL(int pTuner, int LO_Freq, /* R828_Standard_Type */
		...
		this.R828_I2C.RegAddr = 0x1A;
		this.R828_Arry[21] = (byte) (this.R828_Arry[21] & 0xF3);
		this.R828_I2C.Data = this.R828_Arry[21];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
		...
		// Set VCO current = 100
		this.R828_I2C.RegAddr = 0x12;
		this.R828_Arry[13] = (byte) ((this.R828_Arry[13] & 0x1F) | 0x80);
		...
		VCO_fine_tune = (this.R828_I2C_Len.Data[4] & 0x30) >> 4;
		...
		this.R828_I2C.RegAddr = 0x10;
		this.R828_Arry[11] &= 0x1F;
		this.R828_Arry[11] |= (DivNum << 5);
		this.R828_I2C.Data = this.R828_Arry[11];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
		...
		Log.d(TAG,
				"VCO_Freq " + Long.toString(VCO_Freq) + " Nint " + Long.toString(Nint) + " VCO_Fra "
						+ Long.toString(VCO_Fra) + " LO_Freq " + Long.toString(LO_Freq) + " MixDiv "
		...
		if (Nint > 63) {
			Log.d(TAG, String.format("[R820T] No valid PLL values for %u Hz! ", LO_Freq));
			return false;
		}
		...
		Ni = (Nint - 13) / 4;
		Si = Nint - (4 * Ni) - 13;
		this.R828_I2C.RegAddr = 0x14;
		this.R828_Arry[15] = 0x00;
		this.R828_Arry[15] |= (Ni + (Si << 6));
		...
		// pw_sdm
		this.R828_I2C.RegAddr = 0x12;
		this.R828_Arry[13] &= 0xF7;
		if (VCO_Fra == 0) {
			this.R828_Arry[13] |= 0x08;
		}
		this.R828_I2C.Data = this.R828_Arry[13];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		...
		this.R828_I2C.RegAddr = 0x16;
		this.R828_Arry[17] = (byte) SDM16to9;
		this.R828_I2C.Data = this.R828_Arry[17];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
		this.R828_I2C.RegAddr = 0x15;
		this.R828_Arry[16] = (byte) SDM8to1;
		...
		if ((this.Rafael_Chip == R620D) || (this.Rafael_Chip == R828D) || (this.Rafael_Chip == R828)) {
		...
		if ((this.R828_I2C_Len.Data[2] & 0x40) == 0x00) {
			Log.d(TAG, "[R820T] PLL not locked for %u Hz! " + LO_Freq);
			this.R828_I2C.RegAddr = 0x12;
			this.R828_Arry[13] = (byte) ((this.R828_Arry[13] & 0x1F) | 0x60); // increase
						// VCO
						// current
			this.R828_I2C.Data = this.R828_Arry[13];
			if (I2C_Write(pTuner, this.R828_I2C) != true) {
		...
		this.R828_I2C.RegAddr = 0x1A;
		this.R828_Arry[21] = (byte) (this.R828_Arry[21] | 0x08);
		this.R828_I2C.Data = this.R828_Arry[21];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
	...
	boolean R828_RfGainMode(int pTuner, boolean manual) {
		...
		if (manual) {
			// LNA auto off
			this.R828_I2C.RegAddr = 0x05;
			this.R828_Arry[0] = (byte) (this.R828_Arry[0] | 0x10);
			this.R828_I2C.Data = this.R828_Arry[0];
			if (I2C_Write(pTuner, this.R828_I2C) != true) {
				return false;
			}
			...
			this.R828_I2C.RegAddr = 0x0C;
			this.R828_Arry[7] = (byte) ((this.R828_Arry[7] & 0x60) | 0x08);
			this.R828_I2C.Data = this.R828_Arry[7];
			if (I2C_Write(pTuner, this.R828_I2C) != true) {
				return false;
			}
			...
			// LNA
			this.R828_I2C.RegAddr = 0x05;
			this.R828_Arry[0] = (byte) (this.R828_Arry[0] & 0xEF);
			this.R828_I2C.Data = this.R828_Arry[0];
			if (I2C_Write(pTuner, this.R828_I2C) != true) {
				return false;
			}
			...
			// Mixer
			this.R828_I2C.RegAddr = 0x07;
			this.R828_Arry[2] = (byte) (this.R828_Arry[2] | 0x10);
			this.R828_I2C.Data = this.R828_Arry[2];
			if (I2C_Write(pTuner, this.R828_I2C) != true) {
				return false;
			}
	...
	boolean R828_SetFrequency(int pTuner, R828_Set_Info R828_INFO, /* R828_SetFreq_Type */
		...
		// Set TOP,VTH,VTL
		SysFreq_Info1 = R828_SysFreq_Sel(R828_INFO.R828_Standard, (int) R828_INFO.RF_KHz);

		// write DectBW, pre_dect_TOP
		this.R828_Arry[24] = (byte) ((this.R828_Arry[24] & 0x38) | (SysFreq_Info1.LNA_TOP & 0xC7));
		this.R828_I2C.RegAddr = 0x1D;
		...
		this.R828_Arry[23] = (byte) ((this.R828_Arry[23] & 0x07) | (SysFreq_Info1.MIXER_TOP & 0xF8));
		this.R828_I2C.RegAddr = 0x1C;
		this.R828_I2C.Data = this.R828_Arry[23];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		...
		// write LNA VTHL
		this.R828_Arry[8] = (byte) ((this.R828_Arry[8] & 0x00) | SysFreq_Info1.LNA_VTH_L);
		this.R828_I2C.RegAddr = 0x0D;
		...
		// write MIXER VTHL
		this.R828_Arry[9] = (byte) ((this.R828_Arry[9] & 0x00) | SysFreq_Info1.MIXER_VTH_L);
		this.R828_I2C.RegAddr = 0x0E;
		...
		// Cable-1/Air in
		this.R828_I2C.RegAddr = 0x05;
		this.R828_Arry[0] &= 0x9F;
		this.R828_Arry[0] |= SysFreq_Info1.AIR_CABLE1_IN;
		this.R828_I2C.Data = this.R828_Arry[0];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
		...
		// Cable-2 in
		this.R828_I2C.RegAddr = 0x06;
		this.R828_Arry[1] &= 0xF7;
		this.R828_Arry[1] |= SysFreq_Info1.CABLE2_IN;
		this.R828_I2C.Data = this.R828_Arry[1];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
		...
		// CP current
		this.R828_I2C.RegAddr = 0x11;
		this.R828_Arry[12] &= 0xC7;
		this.R828_Arry[12] |= SysFreq_Info1.CP_CUR;
		...
		// div buffer current
		this.R828_I2C.RegAddr = 0x17;
		this.R828_Arry[18] &= 0xCF;
		this.R828_Arry[18] |= SysFreq_Info1.DIV_BUF_CUR;
		this.R828_I2C.Data = this.R828_Arry[18];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
		...
		this.R828_I2C.RegAddr = 0x0A;
		this.R828_Arry[5] = (byte) ((this.R828_Arry[5] & 0x9F) | SysFreq_Info1.FILTER_CUR);
		this.R828_I2C.Data = this.R828_Arry[5];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
		...
		this.R828_Arry[0] = (byte) ((this.R828_Arry[0] & 0x9F) | 0x00);
		this.R828_Arry[1] = (byte) ((this.R828_Arry[1] & 0xF7) | 0x00);
		...
		if (R828_INFO.R828_Standard > SECAM_L1) {
			...
			if (R828_SetFreqMode == FAST_MODE) // FAST mode
			{
				...
				this.R828_Arry[24] = (byte) ((this.R828_Arry[24] & 0xC7) | 0x00); // LNA
						// TOP:lowest
				this.R828_I2C.RegAddr = 0x1D;
				this.R828_I2C.Data = this.R828_Arry[24];
				if (I2C_Write(pTuner, this.R828_I2C) != true) {
				...
				this.R828_Arry[23] = (byte) (this.R828_Arry[23] & 0xFB); // 0:
				// normal
				// mode
				this.R828_I2C.RegAddr = 0x1C;
				this.R828_I2C.Data = this.R828_Arry[23];
				...
				this.R828_Arry[1] = (byte) (this.R828_Arry[1] & 0xBF); // 0:
				// PRE_DECT
				// off
				this.R828_I2C.RegAddr = 0x06;
				this.R828_I2C.Data = this.R828_Arry[1];
				...
				// agc clk 250hz
				this.R828_Arry[21] = (byte) ((this.R828_Arry[21] & 0xCF) | 0x30);
				this.R828_I2C.RegAddr = 0x1A;
				this.R828_I2C.Data = this.R828_Arry[21];
				if (I2C_Write(pTuner, this.R828_I2C) != true) {
					return false;
			...
			} else // NORMAL mode
			{
				...
				// agc clk 250hz
				this.R828_Arry[21] = (byte) ((this.R828_Arry[21] & 0xCF) | 0x30); // 250hz
				this.R828_I2C.RegAddr = 0x1A;
				this.R828_I2C.Data = this.R828_Arry[21];
				if (I2C_Write(pTuner, this.R828_I2C) != true) {
					return false;
				...
				this.R828_Arry[24] = (byte) ((this.R828_Arry[24] & 0xC7) | 0x18); // TOP=3
				this.R828_I2C.RegAddr = 0x1D;
				this.R828_I2C.Data = this.R828_Arry[24];
				if (I2C_Write(pTuner, this.R828_I2C) != true) {
					return false;
				...
				this.R828_Arry[23] = (byte) ((this.R828_Arry[23] & 0xFB) | (SysFreq_Info1.MIXER_TOP & 0x04));
				this.R828_I2C.RegAddr = 0x1C;
				this.R828_I2C.Data = this.R828_Arry[23];
				if (I2C_Write(pTuner, this.R828_I2C) != true) {
					return false;
				...
				this.R828_Arry[25] = (byte) ((this.R828_Arry[25] & 0xE0) | SysFreq_Info1.LNA_DISbyteGE);
				this.R828_I2C.RegAddr = 0x1E;
				this.R828_I2C.Data = this.R828_Arry[25];
				if (I2C_Write(pTuner, this.R828_I2C) != true) {
					return false;
				...
				// agc clk 60hz
				this.R828_Arry[21] = (byte) ((this.R828_Arry[21] & 0xCF) | 0x20);
				this.R828_I2C.RegAddr = 0x1A;
				this.R828_I2C.Data = this.R828_Arry[21];
				if (I2C_Write(pTuner, this.R828_I2C) != true) {
					return false;
			...
			if ((R828_SetFreqMode == NORMAL_MODE) || (R828_SetFreqMode == FAST_MODE)) {
				...
				// write LNA TOP
				this.R828_Arry[24] = (byte) ((this.R828_Arry[24] & 0xC7) | (SysFreq_Info1.LNA_TOP & 0x38));
				this.R828_I2C.RegAddr = 0x1D;
				this.R828_I2C.Data = this.R828_Arry[24];
				if (I2C_Write(pTuner, this.R828_I2C) != true) {
					return false;
				...
				this.R828_Arry[21] = (byte) ((this.R828_Arry[21] & 0xCF) | 0x00);
				this.R828_I2C.RegAddr = 0x1A;
				this.R828_I2C.Data = this.R828_Arry[21];
				if (I2C_Write(pTuner, this.R828_I2C) != true) {
					return false;
				...
				this.R828_Arry[11] = (byte) ((this.R828_Arry[11] & 0xFB) | 0x00);
				this.R828_I2C.RegAddr = 0x10;
				this.R828_I2C.Data = this.R828_Arry[11];
				if (I2C_Write(pTuner, this.R828_I2C) != true) {
					return false;
	...
	boolean R828_SetRfGain(int pTuner, int gain) {
		...
		/* set LNA gain */
		this.R828_I2C.RegAddr = 0x05;
		this.R828_Arry[0] = (byte) ((this.R828_Arry[0] & 0xF0) | lna_index);
		this.R828_I2C.Data = this.R828_Arry[0];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
		...
		/* set Mixer gain */
		this.R828_I2C.RegAddr = 0x07;
		this.R828_Arry[2] = (byte) ((this.R828_Arry[2] & 0xF0) | mix_index);
		this.R828_I2C.Data = this.R828_Arry[2];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
	...
	boolean R828_SetStandard(int pTuner, /* R828_Standard_Type */int RT_Standard) {
		...
		if (this.R828_IMR_done_flag == true) {
			this.R828_Arry[7] = (byte) ((this.R828_Arry[7] & 0xF0) | 0x01 | (this.Xtal_cap_sel << 1));
		} else {
			this.R828_Arry[7] = (byte) ((this.R828_Arry[7] & 0xF0) | 0x00);
		}
		...
		// Record version
		this.R828_I2C.RegAddr = 0x13;
		this.R828_Arry[14] = (byte) ((this.R828_Arry[14] & 0xC0) | VER_NUM);
		...
		if (RT_Standard > SECAM_L1) {
			this.R828_I2C.RegAddr = 0x1D; // [5:3] LNA TOP
			this.R828_I2C.Data = (byte) ((this.R828_Arry[24] & 0xC7) | 0x00);
			if (I2C_Write(pTuner, this.R828_I2C) != true) {
				return false;
			}
		...
		// Filter Calibration
		if (this.R828_Fil_Cal_flag[RT_Standard] == 0) {
			...
			this.R828_Fil_Cal_code[RT_Standard] = (byte) (this.R828_I2C_Len.Data[4] & 0x0F);

			// Filter Cali. Protection
			if ((this.R828_Fil_Cal_code[RT_Standard] == 0) || (this.R828_Fil_Cal_code[RT_Standard] == 15)) {
				if (R828_Filt_Cal(pTuner, Sys_Info1.FILT_CAL_LO, Sys_Info1.BW) != true) {
					return false;
				}
				...
				if (this.R828_Fil_Cal_code[RT_Standard] == 15) {
					this.R828_Fil_Cal_code[RT_Standard] = 0;
				}

			}
			this.R828_Fil_Cal_flag[RT_Standard] = 1;
		}

		// Set Filter Q
		this.R828_Arry[5] = (byte) ((this.R828_Arry[5] & 0xE0) | Sys_Info1.FILT_Q | this.R828_Fil_Cal_code[RT_Standard]);
		this.R828_I2C.RegAddr = 0x0A;
		this.R828_I2C.Data = this.R828_Arry[5];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		...
		this.R828_Arry[6] = (byte) ((this.R828_Arry[6] & 0x10) | Sys_Info1.HP_COR);
		this.R828_I2C.RegAddr = 0x0B;
		this.R828_I2C.Data = this.R828_Arry[6];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		...
		// Set Img_R
		this.R828_Arry[2] = (byte) ((this.R828_Arry[2] & 0x7F) | Sys_Info1.IMG_R);
		this.R828_I2C.RegAddr = 0x07;
		this.R828_I2C.Data = this.R828_Arry[2];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		...
		// Set filt_3dB, V6MHz
		this.R828_Arry[1] = (byte) ((this.R828_Arry[1] & 0xCF) | Sys_Info1.FILT_GAIN);
		this.R828_I2C.RegAddr = 0x06;
		this.R828_I2C.Data = this.R828_Arry[1];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		...
		this.R828_Arry[25] = (byte) ((this.R828_Arry[25] & 0x9F) | Sys_Info1.EXT_ENABLE);
		this.R828_I2C.RegAddr = 0x1E;
		this.R828_I2C.Data = this.R828_Arry[25];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		...
		// Loop through
		this.R828_Arry[0] = (byte) ((this.R828_Arry[0] & 0x7F) | Sys_Info1.LOOP_THROUGH1);
		this.R828_I2C.RegAddr = 0x05;
		this.R828_I2C.Data = this.R828_Arry[0];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		...
		this.R828_Arry[26] = (byte) ((this.R828_Arry[26] & 0x7F) | Sys_Info1.LT_ATT);
		this.R828_I2C.RegAddr = 0x1F;
		this.R828_I2C.Data = this.R828_Arry[26];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		...
		this.R828_Arry[10] = (byte) ((this.R828_Arry[10] & 0x7F) | Sys_Info1.FLT_EXT_WIDEST);
		this.R828_I2C.RegAddr = 0x0F;
		this.R828_I2C.Data = this.R828_Arry[10];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		...
		this.R828_Arry[20] = (byte) ((this.R828_Arry[20] & 0x9F) | Sys_Info1.POLYFIL_CUR);
		this.R828_I2C.RegAddr = 0x19;
		this.R828_I2C.Data = this.R828_Arry[20];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
	...
	boolean R828_Standby(int pTuner, int R828_LoopSwitch) {
		if (R828_LoopSwitch == LOOP_THROUGH) {
			this.R828_I2C.RegAddr = 0x06;
			this.R828_I2C.Data = (byte) 0xB1;
			if (I2C_Write(pTuner, this.R828_I2C) != true) {
				return false;
			}
			this.R828_I2C.RegAddr = 0x05;
			this.R828_I2C.Data = 0x03;
		...
		this.R828_I2C.RegAddr = 0x07;
		this.R828_I2C.Data = 0x3A;
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}

		this.R828_I2C.RegAddr = 0x08;
		this.R828_I2C.Data = 0x40;
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}

		this.R828_I2C.RegAddr = 0x09;
		this.R828_I2C.Data = (byte) 0xC0; // polyfilter off
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}

		this.R828_I2C.RegAddr = 0x0A;
		this.R828_I2C.Data = 0x36;
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}

		this.R828_I2C.RegAddr = 0x0C;
		this.R828_I2C.Data = 0x35;
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}

		this.R828_I2C.RegAddr = 0x0F;
		this.R828_I2C.Data = 0x68; /* was 0x78, which turns off CLK_Out */
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
		...
		this.R828_I2C.RegAddr = 0x17;
		this.R828_I2C.Data = (byte) 0xF4;
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}

		this.R828_I2C.RegAddr = 0x19;
		this.R828_I2C.Data = 0x0C;
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
	...
	boolean R828_Xtal_Check(int pTuner) {
		...
		// cap 30pF & Drive Low
		this.R828_I2C.RegAddr = 0x10;
		this.R828_Arry[11] = (byte) ((this.R828_Arry[11] & 0xF4) | 0x0B);
		this.R828_I2C.Data = this.R828_Arry[11];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
		...
		this.R828_I2C.RegAddr = 0x13;
		this.R828_Arry[14] = (byte) ((this.R828_Arry[14] & 0x80) | 0x7F);
		this.R828_I2C.Data = this.R828_Arry[14];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
		...
		// set auto
		this.R828_I2C.RegAddr = 0x13;
		this.R828_Arry[14] = (byte) (this.R828_Arry[14] & 0xBF);
		this.R828_I2C.Data = this.R828_Arry[14];
		if (I2C_Write(pTuner, this.R828_I2C) != true) {
			return false;
		}
		 ...
		 */
		if (((this.R828_I2C_Len.Data[2] & 0x40) == 0x00) || ((this.R828_I2C_Len.Data[2] & 0x3F) == 0x3F))
		// #endif
		{
			// cap 20pF
			this.R828_I2C.RegAddr = 0x10;
			this.R828_Arry[11] = (byte) ((this.R828_Arry[11] & 0xFC) | 0x02);
			this.R828_I2C.Data = this.R828_Arry[11];
			if (I2C_Write(pTuner, this.R828_I2C) != true) {
				return false;
			}
			...
			// #endif
			{
				// cap 10pF
				this.R828_I2C.RegAddr = 0x10;
				this.R828_Arry[11] = (byte) ((this.R828_Arry[11] & 0xFC) | 0x01);
				this.R828_I2C.Data = this.R828_Arry[11];
				if (I2C_Write(pTuner, this.R828_I2C) != true) {
					return false;
				}
				...
				// #endif
				{
					...
					// #endif
					{
						// X'tal drive high
						this.R828_I2C.RegAddr = 0x10;
						this.R828_Arry[11] = (byte) (this.R828_Arry[11] & 0xF7);
						this.R828_I2C.Data = this.R828_Arry[11];
						if (I2C_Write(pTuner, this.R828_I2C) != true) {
							return false;
						}
						 ...
						 */
						if (((this.R828_I2C_Len.Data[2] & 0x40) == 0x00)
								|| ((this.R828_I2C_Len.Data[2] & 0x3F) == 0x3F))
						// #endif
						{
							return false;
						} else // 0p+high drive lock
						{
							this.Xtal_cap_sel_tmp = XTAL_HIGH_CAP_0P;
						}
					} else // 0p lock
					{
						this.Xtal_cap_sel_tmp = XTAL_LOW_CAP_0P;
					}
				} else // 10p lock
				{
					this.Xtal_cap_sel_tmp = XTAL_LOW_CAP_10P;
				}
			} else // 20p lock
			{
				this.Xtal_cap_sel_tmp = XTAL_LOW_CAP_20P;
			}
		} else // 30p lock
		{
			this.Xtal_cap_sel_tmp = XTAL_LOW_CAP_30P;
		}
